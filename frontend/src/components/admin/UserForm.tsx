...
const UserForm: React.FC<UserFormProps> = ({ user, onBack, onSaved }) => {
  const { email, setEmail, password, setPassword, role, setRole, error, handleSave } = useUserFormLogic(user, onSaved);
      ...
      <div className="bg-gray-900 p-8 rounded-lg border border-gray-800 shadow-xl">
        ...
        <div className="space-y-6">
          <div>
            ...
            <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full bg-black border border-gray-700 rounded p-3 text-white focus:border-blue-500 outline-none" placeholder="correo@ejemplo.com" />
          </div>
          
          <div>
            <label className="block text-sm font-bold text-gray-400 mb-2">Contraseña</label>
            <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} className="w-full bg-black border border-gray-700 rounded p-3 text-white focus:border-blue-500 outline-none" placeholder={user ? "Escribe solo si deseas cambiarla" : "••••••••"} />
          ...
          <div>
            ...
            <select value={role} onChange={(e) => setRole(e.target.value)} className="w-full bg-black border border-gray-700 rounded p-3 text-white focus:border-blue-500 outline-none">
              <option value="student">Alumno</option>
              <option value="teacher">Profesor</option>
            </select>
          </div>

          <button onClick={handleSave} disabled={!email || (!user && !password)} className="w-full bg-blue-600 hover:bg-blue-500 py-3 rounded font-bold flex justify-center items-center gap-2 transition-colors disabled:opacity-50">
